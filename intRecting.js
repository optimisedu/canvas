...
const sketch = () => {
  function generateRandomColor() {
    const hue =
      Math.floor(Math.random() * 460) - Math.floor(Math.random() * 460); //extra hue means going for another rotation on the colorwheel.
  ...
  return ({ context, width, height }) => {
    ...
    points.forEach(([u, v]) => {
      const sqSize = Math.floor(Math.random() * (100 + 1));
      const x = lerp(margin, width - margin, u); //lerp = linear interpolation, center the grid - min, max, t
      const y = lerp(margin, width - margin, v);
      context.beginPath();
      context.rect(x, y, sqSize, Math.floor(Math.random() * (100 + 1)), false);
